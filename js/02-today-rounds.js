    ...
    <div class="today-hero">
      <div onclick="${todaySelectedRound ? `goToRoundDue('${escapeAttr(todaySelectedRound)}')` : `setTab('rounds'); setRoundsView('due');`}" style="cursor:pointer; display:flex; align-items:flex-start; justify-content:space-between; gap:14px;">
        <div>
          ...
          ${todaySelectedRound ? `<div class="lbl" style="margin-top:8px; color:#fff;">${escapeHtml(todaySelectedRound)}</div>` : ''}
    ...
    <div class="today-grid">
      ...
      <div class="today-tile" onclick="openMileageEntry();" style="display:flex; align-items:flex-start; justify-content:space-between; gap:10px;">
        <div style="min-width:0;">
          <div style="font-size:0.8125rem; font-weight:800; color:var(--ink);">Mileage</div>
...
function avgPricePerHouseForType(list, cat){
  const inCat = list.filter(x => (x.propertyType || 'Not recorded') === cat);
  if(!inCat.length) return null;
  const weight = inCat.reduce((s,x)=>s+houseWeight(x), 0);
  if(!weight) return null;
  const total = inCat.reduce((s,x)=>s+Number(x.price||0),0);
  return total/weight;
}
// The benchmark a customer's price is judged against, for both the "💷 Low"
// badge and the Price review report: the average £-per-house for their own
// property type, within their own round if that round has 4+ active
// customers of that type — otherwise falling back to the average for that
// type across every round, so a small or mixed round can still be judged
// sensibly rather than just being skipped.
function priceReviewBenchmark(c){
  const cat = c.propertyType || 'Not recorded';
  const roundName = c.round || 'Unassigned';
  const roundTypeCusts = data.customers.filter(x=>!x.paused && (x.round||'Unassigned')===roundName && (x.propertyType||'Not recorded')===cat);
  if(roundTypeCusts.length >= 4){
    return avgPricePerHouseForType(roundTypeCusts, cat);
  }
  const globalTypeCusts = data.customers.filter(x=>!x.paused && (x.propertyType||'Not recorded')===cat);
  return avgPricePerHouseForType(globalTypeCusts, cat);
}
...
function openRoundActionsMenu(rn){
  openSheet(`
    ...
    <button class="backup-btn" onclick="closeSheet(); openRoundUpliftSheet('${escapeAttr(rn)}')">
      ...
      <div><div class="t1">Apply price uplift</div><div class="t2">Increase every active customer's price by a % or flat amount</div></div>
    </button>
...
function custCardHtml(c){
  ...
  // Judged against the average for this customer's own property type — see
  // priceReviewBenchmark for the round-then-global fallback logic.
